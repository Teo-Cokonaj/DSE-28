...
class Configurations_landing_gear_sizing():

    def __init__(self, L1, L2, L3, x_cg, diameter_fuselage, wing_span):

        self.L1 = L1
        self.L2 = L2
        self.L3 = L3
        self.x_cg = x_cg
        self.up_sweep_angle_rad = 15 * np.pi/180
        self.diameter_fuselage = diameter_fuselage
        self.wing_height_from_centre_line = diameter_fuselage /2
        self.wing_span = wing_span


    def lg_pos_and_length(self, debug=False):

        sigma = self.up_sweep_angle_rad
        theta = 15 * np.pi / 180 # also airfoil stall angle
        phi_min =  7.0 * np.pi / 180  

        x_cone_start = self.L1 + self.L2
        x_tail_tip = self.L1 + self.L2 + self.L3
        R = self.diameter_fuselage / 2

        def tail_point_near(l_landing_gear):
            return (x_cone_start, l_landing_gear - R)

        def tail_point_far(l_landing_gear):
            return (x_tail_tip, l_landing_gear - R + self.L3 * np.tan(sigma))
        ...
        def nose_gear_load_fraction(x_main_lg, l_landing_gear):
            ...
            return (x_main_lg - self.x_cg) / (x_main_lg - x_nlg)

        def turn_over_angle(x_main_lg, Y_lg, l_landing_gear):
            x_nlg = nose_gear_pos(l_landing_gear)
            d = x_main_lg - x_nlg
            alpha = np.arctan2(Y_lg, d)                       
            c = (self.x_cg - x_nlg) * np.sin(alpha)
            return np.arctan2(c, l_landing_gear)
        
        def wing_tip_to_lg_angle(Y_lg, l_landing_gear):
            vertical   = l_landing_gear + self.wing_height_from_centre_line
            horizontal = (self.wing_span / 2) - Y_lg
        ...
        def constraint_wing_tip_clearance(v):
            l_landing_gear, x_main_lg, Y_lg = v
            phi = wing_tip_to_lg_angle(Y_lg, l_landing_gear)
            return phi - phi_min   
        ...
        def constraint_nose_gear_behind_cg(v):
            # Keeps the optimization physically realistic: nose gear stays ahead of the CG
            l_landing_gear, x_main_lg, Y_lg = v
            return self.x_cg - nose_gear_pos(l_landing_gear)
        ...
        landing_gear_info = {'strut_length' : l_opt,
                             'MLG_x_position' : x_mlg_opt,
                             'MLG_lateral_track' : Y_lg_opt,
                             'NLG_x_position' : x_nlg_opt}

        return landing_gear_info
