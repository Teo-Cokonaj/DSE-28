...
class TailFinder(EmpennageFinder):
    def __init__(self, fixed, AR_h:float=3., taper_h:float=.7, taper_v:float=.9, SM=0.05):
    ...
    def find_planforms(self, main_wing:Planform, initial:float=.1, maxiter:int=50, tolerance:float=1e-4) -> list[Planform]:
        ...
        for i in range(maxiter):
            ...
            Sh_S_new_stab = (x_cg_mac_max - x_ac_mac - self.SM) / (horizontal_tail.CL_alpha * l_h_mac / main_wing.CL_alpha)
