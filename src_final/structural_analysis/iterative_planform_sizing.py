...
def find_planform_thickness(planform:Planform, thicknesses:list[float], fuselage_diameter:float, material:Material, load_factor:float=6., load_factor_maneuver:float=1.) -> float:
    for thickness in thicknesses:
        ...
        if (np.max(shear_stress) < material.fracture_strength / 3) and (np.min(are_we_buckling) > 0) and (np.max(np.abs(normal_stress)) < material.fracture_strength / 1.5) and (np.max(np.abs(twist_deg))<10.) and (np.max(np.abs(deflection_m)) < .15*planform.span):
            return thickness
...
def find_planform_mass_cg(planform:Planform, thickness:float, density_core:float, density_skin:float, number_of_sections:int=20, safety_factor = 1.5)->tuple[float, float]:
    ...
    crossec_area_stations = np.pi * (chord_stations * planform.thickness_to_chord - 2*thickness) * (chord_stations - 2*thickness) / 4
