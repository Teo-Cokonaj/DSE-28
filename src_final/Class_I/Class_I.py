...
class Class_I:
    def __init__(self, altitude_cruise:float,
                 altitude_go_around:float,
                 efficiency_engine_cruise:float,
                 energy_density_saf:float,
                 time_half_turn:float,
                 debug=False,
                 efficiency_engine_go_around:float=None,
                 efficiency_engine_mach_max:float=None):
    ...
    def run_estimation(self, oem_fraction:float, CL_max_glide_ratio_go_around:float, glide_ratio_mach_max:float, 
                       glide_ratio_cruise:float, glide_ratio_go_around:float, airspeed_approach:float, wing_loading:float,
                       assumptions: Assumptions,) -> Class_I_Result:
        ...
        mtom = assumptions.mass_payload / (1 - oem_fraction - fuel_fraction)
