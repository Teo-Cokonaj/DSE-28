...
class OEMStep(DesignOptionStep):
    ...
    def update(self, state:DesignOptionState) -> DesignOptionStateIterable:
        ...
        if state.fixed.choices.canard_capability: #there is no case of movable port and no canard
            ...
            W = state.iterable.aircraft_parameters.total_mass*CONSTANTS.G0*CONSTANTS.ULTIMATE_LOAD_FACTOR
            ...
            if state.fixed.choices.main_wing_x_movable:
                ...
                if self.plot:
                    print("branch hit")
                    plt.subplot(211)
                    plt.plot(x, max_shear-shear)
                    plt.subplot(212)
                    plt.plot(x, max_moment-moment)
                    plt.show()
                ...
                if self.plot:
                    plotReqThickness(x, t_skin_canard_variable)
