...
from Mission_Segment import Mission_Segment
...
def fuel_mass_fraction(altitude_go_around:float, altitude_cruise:float, time_half_turn:float, CL_max_glide_ratio_go_around:float, 
    ...
    #determining missed approach parameters
    ...
    # glide_ratio:float, airspeed:float, time:float, altitude:float, airspeed_initial:float=0.
    segment_cruise = Mission_Segment(glide_ratio_cruise, airspeed_cruise, CONSTANTS.TIME_CRUISE, altitude_cruise),
    segment_mach_max = Mission_Segment(glide_ratio_mach_max, airspeed_mach_max, CONSTANTS.TIME_MACH_MAX, CONSTANTS.ALTITUDE_MACH_MAX, airspeed_cruise),
    ...
    segment_go_around.R *= CONSTANTS.N_LANDING_ATTEMPTS #we need multiple times the range for one go around
